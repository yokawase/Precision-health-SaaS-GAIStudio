          ...
          {symptoms.map(s => (
            <label key={s.id} className="flex items-center p-3 border border-slate-200 rounded-lg cursor-pointer hover:bg-slate-50 transition-colors">
               <input 
                 ...
                 checked={activeSymptoms.includes(s.id)}
        ...
        <div className="space-y-4">
          {activeSymptoms.includes('fever') && (
            <SymptomCard 
              ...
              redDesc={["38℃以上の発熱が4日以上持続", "強い頭痛、項部硬直(首が固い・髄膜炎疑い)", "けいれん、意識障害、呼吸困難"]}
            />
          )}
          {activeSymptoms.includes('pain') && (
            <SymptomCard 
              ...
              redDesc={["急性の引き裂かれるような背部痛 (大動脈解離の可能性)", "原因不明の体重減少、排尿・排便障害(馬尾神経圧迫)", "安静にしていても痛む重度の夜間痛"]}
            />
          )}
          {activeSymptoms.includes('stomach') && (
            <SymptomCard 
              ...
              redDesc={["激しい腹痛の持続", "血便・吐血 (タール便含む)", "強い脱水症状 (意識レベル低下、尿量減少)"]}
            />
          )}
          {activeSymptoms.includes('injury') && (
            <SymptomCard 
              ...
              redDesc={["圧迫しても止血困難", "砂や異物が完全に除去できない (感染・刺青リスク)", "広範囲の熱傷、関節が動かせない"]}
            />
          )}
        </div>
      </div>
  ...
  <div className="border border-slate-300 rounded-lg overflow-hidden">
     ...
     <div className="p-4 bg-white">
        <div className="mb-4 bg-emerald-50 border-l-4 border-emerald-500 p-3 rounded-r">
           <div className="font-bold text-emerald-700 flex items-center gap-2 mb-1">
             <ShieldCheck className="w-4 h-4"/> セルフケア適用
           </div>
        ...
        <div className="bg-red-50 border-l-4 border-red-500 p-3 rounded-r">
           <div className="font-bold text-red-700 flex items-center gap-2 mb-1">
             <AlertCircle className="w-4 h-4"/> 受診推奨 (Red Flag)
           </div>
